  ...
  (email, password, isRemeberMe) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/api/v1/auth/loginUsingEmail`,
        {
          email,
          password,
        },
      ...
      cookies.set('ddAdminToken', data.data.user.token, {
        path: '/',
        expires: updatedDate,
      });
    } catch (error) {
...
export const adminRegister =
  (firstName,lastName, email,phone,gender,country, password, navigate) => async (dispatch) => {
    try {
      dispatch({
